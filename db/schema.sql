...
-- Admin users table
CREATE TABLE IF NOT EXISTS ent_admins (
...
-- Students table
CREATE TABLE IF NOT EXISTS ent_students (
  id INT AUTO_INCREMENT PRIMARY KEY,
...
-- Exams table
CREATE TABLE IF NOT EXISTS ent_exams (
  id INT AUTO_INCREMENT PRIMARY KEY,
...
-- Sections table
CREATE TABLE IF NOT EXISTS ent_sections (
  id INT AUTO_INCREMENT PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS ent_questions (
  ...
  sort_order INT DEFAULT 0,
  FOREIGN KEY (exam_id) REFERENCES ent_exams(id) ON DELETE CASCADE,
  FOREIGN KEY (section_id) REFERENCES ent_sections(id) ON DELETE SET NULL
);
...
CREATE TABLE IF NOT EXISTS ent_responses (
  id INT AUTO_INCREMENT PRIMARY KEY,
  ...
  FOREIGN KEY (question_id) REFERENCES ent_questions(id) ON DELETE CASCADE,
  UNIQUE KEY unique_response (session_id, question_id)
);

-- Violations table
CREATE TABLE IF NOT EXISTS ent_violations (
  id INT AUTO_INCREMENT PRIMARY KEY,
  session_id INT NOT NULL,
...
CREATE TABLE IF NOT EXISTS ent_proctoring_logs (
  id INT AUTO_INCREMENT PRIMARY KEY,
  session_id INT NOT NULL,
...
CREATE TABLE IF NOT EXISTS ent_results (
  ...
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  FOREIGN KEY (session_id) REFERENCES ent_sessions(id) ON DELETE CASCADE,
  FOREIGN KEY (reviewed_by) REFERENCES ent_admins(id) ON DELETE SET NULL
);
...
INSERT INTO ent_exams (title, description, duration_minutes, total_questions, max_tab_switches, max_violations) VALUES
...
INSERT INTO ent_sections (exam_id, title, sort_order) VALUES
(1, 'Section A - Visual Perception', 1),
(1, 'Section B - Design Thinking', 2),
(1, 'Section C - Creative Aptitude', 3);

-- Insert sample questions
INSERT INTO ent_questions (exam_id, section_id, question_text, option_a, option_b, option_c, option_d, correct_option, sort_order) VALUES
